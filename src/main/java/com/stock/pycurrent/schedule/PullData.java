    ...
    private void checkRealData(String[] codes, Map<String, Map<String, EmConstantValue>> stockMap, Map<String, List<BigDecimal>> codePctMap, Map<String, List<String>> logsMap, List<EmRealTimeStock> stockList) {
        ...
        for (EmRealTimeStock rt : stockList) {
            ...
            if (rt.getCurrentPri() != null && rt.getVol() != null && (tsCode.startsWith("0") || tsCode.startsWith("60")) && !tsName.contains("退") && !noConcerned && (limitCodeMap.containsKey(tsCode) && (limitCodeMap.get(tsCode).getCount() > 2 || (limitCodeMap.get(tsCode).getCount() == 2 && CalculateUtils.reachTenLimit(rt))))) {
                logsMap.get("A").add(fixPositiveLength(limitCodeMap.get(tsCode).getCount(), 2) + " " + (tsCode.charAt(0) + tsCode.substring(2, 6)) + fixLength(nameFirst, 3) + fixLength(rt.getPctChg(), 6) + fixLength(rt.getChangeHand(), 5) + fixLength("", 7) + fixLength("", 10) + fixLength("", 10) + fixLength(rt.getCurrentPri(), 6) + fixLength("", 9) + fixLength("", 11) + fixLength("", 8) + fixLength(rt.getCirculationMarketCap().divide(HUNDRED_MILLION, 3, RoundingMode.HALF_UP), 8));
            }
            ...
            if (!noConcerned && !tsName.contains("退") && tsCode.startsWith("3") && rt.getPctChg() != null && (highLimit || concerned || holds || rangeOverLimit || yesterdayHigh)) {
                ...
                BigDecimal avg = amountMap.containsKey(tsCode) ? amountMap.get(tsCode).compareTo(rt.getAmount()) == 0 || rt.getVol() - volMap.get(tsCode) == 0 ? BigDecimal.ZERO : rt.getAmount().subtract(amountMap.get(tsCode)).divide(BigDecimal.valueOf(rt.getVol() - volMap.get(tsCode)).multiply(HUNDRED), 3, RoundingMode.HALF_UP) : rt.getVol() == null ? BigDecimal.ZERO : rt.getAmount().divide(BigDecimal.valueOf(rt.getVol()).multiply(HUNDRED), 3, RoundingMode.HALF_UP);
                long volStep = 0L;
                String volStr = "";
                if (rt.getVol() != null) {
        ...
        log.warn(" ________________________________________________________________________________________________________________________________________________");
