...
class SDateThread(threading.Thread):
    ...
    def run(self):
        if not self.symbol.startswith('00') and not self.symbol.startswith('30') and not self.symbol.startswith('60'):
            return
        conn = pool.connection()
        cursor = conn.cursor()
        try:
            em_params["stock"] = self.symbol
            em_params["market"] = 'sh' if self.symbol.startswith('60') else 'sz'
            ...
            df.insert(loc=1, column='代码', value=self.symbol)
