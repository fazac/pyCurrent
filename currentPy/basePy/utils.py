...
def test_proxies(proxies):
    ...
    for i in range(len(proxies)):
        ...
        try:
            ...
            r = requests.get(url='https://www.example.com', headers={'User-Agent': UserAgent().random, },
                             proxies={"http": "http://{}".format(proxies[i])}, timeout=5)
            end = time.time()
            t = end - start
            busy_time.append(t)
            if t > 3:  # 如果响应时间超过3秒
...
def test_connection(IP):
    ...
    for i in range(length):
        try:
            ...
            # 发送带有负载的GET请求
            r = requests.get('https://www.example.com', proxies={"http": "http://{}".format(IP[i])},
                             headers={'User-Agent': UserAgent().random}, timeout=10)
            # 计算响应时间
...
def test_proxy(IP, proxy_response_times, lock, MAX_Times):
    try:
        ...
        for j in range(MAX_Times):
            ...
            r = requests.get('https://www.example.com', proxies={"http": f"http://{IP}"},
                             headers={'User-Agent': UserAgent().random, }, timeout=5)
            if r.status_code == 200:
                r.encoding = r.apparent_encoding  # Solve garbled characters
                page = r.text
                soup = BeautifulSoup(page, 'lxml')
                title = soup.find(
                    'h1').text  # Generally, if the h1 tag is not found here, it will report an error and enter except
                if title == 'Example Domain':
                    if (j + 1) % 10 == 0:
